 ...
 */
class Modifiers {
    static final int ACC_IDENTITY = 0x0020; // Note: same as ACC_SYNCHRONIZED
    ...
    static int toInterface(int bitmask) {
        return (bitmask | (Modifier.INTERFACE | Modifier.ABSTRACT)) &
            (~Modifier.FINAL & ~ACC_IDENTITY);
    }

    // For classes.
    static int toAbstractClass(int bitmask) {
        return (bitmask | Modifier.ABSTRACT) & ~Modifier.FINAL;
    }

    // For methods.
    static int toAbstractMethod(int bitmask) {
    ...
    static int toModule(int bitmask) {
        return (bitmask | 0x8000) & ~ACC_IDENTITY;
    }

    // For classes.
    static int toValueClass(int bitmask) {
        return (bitmask | Modifier.FINAL) &
            (~Modifier.INTERFACE & ~Modifier.ABSTRACT & ~ACC_IDENTITY);
    }
    ...
    static boolean isStrictInstance(int bitmask) {
        return (bitmask & (0x0800 | Modifier.STATIC)) == 0x0800;
    }
}
